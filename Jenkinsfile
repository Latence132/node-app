    ...
    stages {
        ...
        stage('Deploy to k8s') {
            steps {
                ...
                sshagent(['kops']) {
                    sh 'scp -o StrictHostKeyChecking=no services.yml node-app-pod.yml ec2-user@35.180.122.109:/home/ec2-user/'
                    script {
                        try {
                            sh 'ssh ec2-user@35.180.122.109 kubectl apply -f .'
                    }catch (error) {
                            sh 'ssh ec2-user@35.180.122.109 kubectl create -f .'
                        }
                    }
                }
            }
